...
\TLV
   ...
   m4_asm(ADDI, r20, r0, 101)
   m4_asm(ADDI, r21, r0, 11111111)
   m4_asm(LUI, r22, 00000000000001011000)
   ...
   |cpu
      ...
      @1
         $instr[31:0] = (*cyc_cnt == 20) ? 32'h015A100B :
                        (*cyc_cnt == 21) ? 32'h000B0B8B :
                        $imem_rd_data[31:0];
         ...
         $is_config_lut     = $dec_bits ==? 11'bx_001_0001011;  
         $is_lut_activation = $dec_bits ==? 11'bx_000_0001011;  

      @2
         $br_tgt_pc[31:0]   = $pc[31:0] + $imm[31:0];
         $jalr_tgt_pc[31:0] = $src1_value[31:0] + $imm[31:0];

         $src1_value[31:0] = (>>1$rf_wr_en && (>>1$rd == $rs1)) ? >>1$result[31:0] :
                             (>>2$rf_wr_en && (>>2$rd == $rs1)) ? >>2$result[31:0] :
                             $rf_rd_data1[31:0];
         $src2_value[31:0] = (>>1$rf_wr_en && (>>1$rd == $rs2)) ? >>1$result[31:0] :
                             (>>2$rf_wr_en && (>>2$rd == $rs2)) ? >>2$result[31:0] :
                             $rf_rd_data2[31:0];
         $rf_rd_en1 = $rs1_valid;
